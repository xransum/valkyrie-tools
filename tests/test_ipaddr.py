...
class TestIsIpv4Addr(unittest.TestCase):
    ...
    def test_leading_zeros_ipv4_address(self) -> None:
    ...
    def test_non_string_input(self) -> None:
        ...
        ipaddr = 12345
        result = is_ipv4_addr(ipaddr)  # type: ignore[arg-type]
        self.assertFalse(result)
...
class TestIsValidIpAddr(unittest.TestCase):
    ...
    def test_value_isinstance_false(self, mock_isinstance: MagicMock) -> None:
    ...
    def test_value_value_error(self, mock_ip_address: MagicMock) -> None:
        """Returns False when isinstance is False."""
        # mock_isinstance.return_value = False
...
class TestIsPrivateIp(unittest.TestCase):
    ...
    def test_is_private_ipv4_in_cidr_range(self) -> None:
        """True a private IPv4 address in a private CIDR range."""
        ipaddr = "192.168.0.1"
        result = is_private_ip(ipaddr)
        self.assertTrue(result)

    def test_is_private_ipv6_in_cidr_range(self) -> None:
        """True a private IPv6 address in a private CIDR range."""
        ipaddr = "fd00::1"
        result = is_private_ip(ipaddr)
        self.assertTrue(result)

    def test_is_public_ipv4_address(self) -> None:
        """False a public IPv4 address."""
        ipaddr = "8.8.8.8"
        result = is_private_ip(ipaddr)
        self.assertFalse(result)

    def test_is_ipv4_not_in_cidr_range(self) -> None:
        """False an IPv4 address that is not in a private CIDR range."""
        ipaddr = "123.123.123.123"
        result = is_private_ip(ipaddr)
        self.assertFalse(result)

    def test_is_ipv6_not_in_cidr_range(self) -> None:
        """False an IPv6 address that is not in a private CIDR range."""
        ipaddr = "2001:db8::1"
        result = is_private_ip(ipaddr)
        self.assertFalse(result)

    def test_is_ipv4_mapped_ipv6_not_in_cidr_range(self) -> None:
        """False an IPv4-mapped IPv6 address not in a private CIDR range."""
        ipaddr = "::ffff:192.0.2.1"
        result = is_private_ip(ipaddr)
        self.assertFalse(result)

    def test_is_invalid_ip_address(self) -> None:
        """False an invalid IP address string."""
        ipaddr = "invalid_ip_address"
        result = is_private_ip(ipaddr)
        self.assertFalse(result)

    def test_is_returns_false_for_public_ipv6_address(self) -> None:
        """False a public IPv6 address."""
        ipaddr = "2001:0db8:85a3:0000:0000:8a2e:0370:7334"
        result = is_private_ip(ipaddr)
        self.assertFalse(result)

    def test_is_returns_false_for_public_ipv4_address(self) -> None:
        """False a public IPv4 address."""
        ipaddr = "203.0.113.1"
        result = is_private_ip(ipaddr)
        self.assertFalse(result)

    def test_is_returns_false_for_non_private_ipv4_address(self) -> None:
        """False an IPv4 address that is not in a private CIDR range."""
        ipaddr = "100.1.23.1"
        result = is_private_ip(ipaddr)
        self.assertFalse(result)

    def test_is_ipv6_with_zone_identifier_not_in_private_cidr_range(
        self,
    ) -> None:
    ...
    def test_is_private_ipv4_in_different_cidr_range(self) -> None:
        """Alt 3: Test a private IPv4 address in a different private CIDR range."""
        ipaddr = "172.16.0.1"
        result = is_private_ip(ipaddr)
        self.assertTrue(result)

    def test_is_private_ipv4_in_multiple_cidr_ranges(self) -> None:
        """Alt 4: Test a private IPv4 address in multiple private CIDR ranges."""
        ipaddr = "192.168.0.1"
        result = is_private_ip(ipaddr)
        self.assertTrue(result)

    def test_is_private_ipv6_in_different_cidr_range(self) -> None:
...
class TestGetIpInfo(unittest.TestCase):
    ...
    def test_get_ip_info(self) -> None:
    ...
    def test_get_ip_info_invalid_ip(
        self, mock_is_valid_ip_addr: MagicMock
    ) -> None:
    ...
    def test_exception_on_status(self, mock_get: MagicMock) -> None:
...
class TestGetTorNodeIpAddrs(unittest.TestCase):
    ...
    @patch("valkyrie_tools.ipaddr.requests.get")
    def test_get_tor_node_ip_addrs(self, mock_get: MagicMock) -> None:
    ...
    @patch("valkyrie_tools.ipaddr.requests.get")
    def test_get_tor_node_ip_addrs_http_error(
    ...
    @patch("requests.get")
    def test_get_tor_node_ip_addrs_cached(self, mock_get: MagicMock) -> None:
...
class TestIsIpTorNode(unittest.TestCase):
    ...
    def test_ip_is_tor_node(
        self, mock_get_tor_node_ip_addrs: MagicMock
    ) -> None:
    ...
    @patch("valkyrie_tools.ipaddr.get_tor_node_ip_addrs")
    def test_cache_refresh(self, mock_get_tor_node_ip_addrs: MagicMock) -> None:
...
class TestGetAwsIpRanges(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        get_aws_ip_ranges.clear_cache()  # type: ignore[attr-defined]  # noqa: B950
    ...
    @patch("valkyrie_tools.ipaddr.requests.get")
    def test_get_aws_ip_ranges(self, mock_get: MagicMock) -> None:
    ...
    @patch("valkyrie_tools.ipaddr.requests.get")
    def test_get_aws_ip_ranges_http_error(self, mock_get: MagicMock) -> None:
...
class TestIsAwsIpAddr(unittest.TestCase):
    ...
    def test_is_aws_ipv4_addr(self, mock_get_aws_ip_ranges: MagicMock) -> None:
        """Test the is_aws_ip_addr function."""
        # Arrange
    ...
    @patch("valkyrie_tools.ipaddr.get_aws_ip_ranges")
    def test_is_aws_ipv6_addr(self, mock_get_aws_ip_ranges: MagicMock) -> None:
        """Test the is_aws_ip_addr function."""
        # Arrange
        mock_get_aws_ip_ranges.return_value = self.mock_results
        # Act
    ...
    @patch("valkyrie_tools.ipaddr.get_aws_ip_ranges")
    def test_is_aws_invalid_ip_addr(
        self, mock_get_aws_ip_ranges: MagicMock
    ) -> None:
        """Test the is_aws_ip_addr function."""
        # Arrange
        mock_get_aws_ip_ranges.return_value = self.mock_results
...
class TestGetCloudflareRange(unittest.TestCase):
    ...
    @patch("valkyrie_tools.ipaddr.requests.get")
    def test_failed_request(self, mock_get: MagicMock) -> None:
...
class TestGetCloudflareIpRanges(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        get_cloudflare_ip_ranges.clear_cache()  # type: ignore[attr-defined]  # noqa: B950

    @patch("valkyrie_tools.ipaddr.get_cloudflare_range")
    def test_get_cloudflare_ip_ranges_success(
        self, mock_get_cloudflare_range: MagicMock
    ) -> None:
    ...
    @patch("valkyrie_tools.ipaddr.get_cloudflare_range")
    def test_get_cloudflare_ip_ranges_failure(
        self, mock_get_cloudflare_range: MagicMock
    ) -> None:
...
class TestIsCloudflareIpAddr(unittest.TestCase):
    ...
    @patch("valkyrie_tools.ipaddr.get_cloudflare_range")
    def test_is_cloudflare_ipv4_addr(
        self, mock_get_cloudflare_range: MagicMock
    ) -> None:
        """Test the is_cloudflare_ip_addr function."""
        # Arrange
    ...
    @patch("valkyrie_tools.ipaddr.get_cloudflare_range")
    def test_is_cloudflare_ipv6_addr(
        self, mock_get_cloudflare_range: MagicMock
    ) -> None:
        """Test the is_cloudflare_ip_addr function."""
        # Arrange
    ...
    @patch("valkyrie_tools.ipaddr.get_cloudflare_range")
    def test_is_cloudflare_invalid_ip_addr(
        self, mock_get_cloudflare_range: MagicMock
    ) -> None:
        """Test the is_cloudflare_ip_addr function."""
        # Arrange
    ...
    def test_is_cloudflare_value_error(self, mock_ipaddress: MagicMock) -> None:
        """Test the is_cloudflare_ip_addr function."""
        # Arrange
        mock = Mock(side_effect=ValueError("foo"))
...
class TestGetFastlyIpRanges(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        get_fastly_ip_ranges.clear_cache()  # type: ignore[attr-defined]  # noqa: B950

    @patch("valkyrie_tools.ipaddr.requests.get")
    def test_get_fastly_ip_ranges_success(self, mock_get: MagicMock) -> None:
    ...
    @patch("valkyrie_tools.ipaddr.requests.get")
    def test_get_fastly_ip_ranges_failure(self, mock_get: MagicMock) -> None:
...
class TestIsFastlyIpAddr(unittest.TestCase):
    ...
    def test_is_fastly_ipv4_addr(
        self, mock_get_fastly_ip_ranges: MagicMock
    ) -> None:
        """Test the is_fastly_ip_addr function."""
        # Arrange
    ...
    @patch("valkyrie_tools.ipaddr.get_fastly_ip_ranges")
    def test_is_fastly_invalid_ip_addr(
        self, mock_get_fastly_ip_ranges: MagicMock
    ) -> None:
        """Test the is_fastly_ip_addr function."""
        # Arrange
